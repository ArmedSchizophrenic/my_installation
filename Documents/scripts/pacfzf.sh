#!/bin/bash
# Searches repos and installs multiple packages. 
# Currently works with Debian, Ubuntu and Arch, and 
# experimental support for Fedora and Void.
...
case "$distro" in
  debian|ubuntu|Android) debian "$request" ;;
  arch) arch "$request" ;;
  manjaro) arch "$request" ;;
  void) void "$request" ;;
